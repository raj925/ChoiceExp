...
class DoubleDotGrid {
     ...
     * @param {array} [args.forcedTrials] - perm array for forced trials
     * @param {array} [args.choiceTrials] - perm array for choice trials
     */
    constructor(nDotsL, nDotsR, args = {}) {
        ...
        this.forcedTrials = args.forcedTrials;
        this.choiceTrials = args.choiceTrials;
        this.blk4Trials = args.blk4Trials;
        this.currentTrialType = args.currentTrialType;
        ...
        this.gridL = this.renewGrid(this.dotCountL,0);
        this.gridR = this.renewGrid(this.dotCountR,1);
     ...
     */
    renewGrid(dotCount, dir) {
        // Find the current dot difference.
        // let dd = Math.abs(dotCount - 200);
        // // Advisor tables support a maximum dot difference of 15.
        // if (dd > 15)
        // {
        //     dd = 15;
        // }
        // let grid = [];
        // // First, create a grid of zeroes.
        ...
        // let arrayCount = 0;
        // // Now we convert the 1 dimensional array of 1s and 0s into the 2x2 grid.
        // for(let i=0; i<y; i++)
        // {
        //     for(let j=0; j<x; j++)
        //     {
        //         if(array[arrayCount] == 1)
        //         {
        //             grid[j][i] = 1;
        //         }
        //         arrayCount++;
        //     }
        // }
        // // We always call the below code once for the left and right grid.
        // // This is so that we only move on to the next dot config once we have plotted both grids.
        // // We need the perm array index to refer to the dot config for both the left and right grids.
        // // This is why we have this dir variable, set to 0 for left and 1 for the right grid.
        // if (dir == 1)
        // {
        //     if (this.currentTrialType == 1)
        //     {
        //         var removedElement = (this.forcedTrials).shift();
        //     }
        //     else if (this.currentTrialType == 2)
        //     {
        //         var removedElement = (this.choiceTrials).shift();
        //     }
        //     else if (this.currentTrialType == 5)
        //     {
        //         var removedElement = (this.blk4Trials).shift();
        //     }
...
// Below object is not currently used.
/**
...
// ADVISOR is currently not in use, refer to the Cue object below instead.
/**
...
class Advisor {
    ...
    constructor(id, adviceType, voice = null, portrait = 0, styleClass = '', args = {}) {
        if(typeof id !== 'object') {
            ...
            if (id == 1)
            {
                this.portraitSrc = 'assets/image/picname.jpg';
            }

            else if (id == 2)
            {
                this.portraitSrc = 'assets/image/computer.jpg';
            }
        } else {
...
class Cue extends Advisor {
    ...
    constructor(id, adviceType, voice = null, portrait = 0, styleClass = '', args = {}) {
        ...
        // We set the images used for each advisor (eg human and computer)
        if (id == 1)
        {
    ...
    draw(div, options = {}) {
        ...
        picDiv.id = 'jspsych-jas-present-advice-image' + idSuffix;
        picDiv.classList.add('jspsych-jas-present-advice-image');
        if (document.getElementById("jspsych-jas-change-advisors-wrapper") == null)
        {
            picDiv.style.transform = "translate(0%,-5%)";
        }
        else
        {
            picDiv.style = "";
...
class Trial {
    constructor(id, args = {}) {
        ...
        this.advisorAnswer = typeof args.advisorAnswer === 'undefined'? null: args.advisorAnswer;
        this.advisorCorrect = typeof args.advisorCorrect === 'undefined'? null: args.advisorCorrect;
        this.advisorConfidence = typeof args.advisorConfidence === 'undefined'? null: args.advisorConfidence;
...
class Governor {
    ...
    drawProgressBar() {
        if (document.querySelector('#jspsych-progressbar-container') === null) {
            ...
            let prac = this.pracBlocks;
            let exp = this.expBlocks;
            for (let x = 0; x < (prac + exp)-1; x++)
            {
                let n  = 0;
                if (x >= prac)
                {
                    n = n + prac*this.pracTrials;
                    n = n + ((x - prac)+1)*this.expTrials;
                }
                else
                {
                    n = n + (x+1)*this.pracTrials;
                }
                let outEle = document.querySelector('#progressbar-outer');
                outEle.style.position = 'relative';
                let pAdd = document.createElement('p');
                let pTag = outEle.appendChild(pAdd);
                pTag.innerHTML = '|';
                pTag.style = "position: absolute;z-index: 1;margin-top: -2em;font-size:15px; margin-left: -0.2em;left: " + ((n+1) / this.numOfTrials * 100).toString() + "%";
            }
        }
        if ( this.currentTrialIndex !== this.numOfTrials)
        {
            let inner = document.querySelector('#progressbar-inner');
            inner.style.width = ((this.currentTrialIndex + 1) / this.numOfTrials * 100).toString()+'%';
            let numDiv = document.querySelector('#progressbar-number');
            numDiv.innerHTML = (this.currentTrialIndex + 1) + ' / ' + this.numOfTrials;
