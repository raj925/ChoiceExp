...
##                        break
            ...
            if (estFlag == 1):
                for estimateFile in os.listdir('../EstimateData'):
                    estimateName = pid + '_ESTIMATE.csv'
                    if fnmatch.fnmatch(estimateFile, estimateName):
                        ...
                        with open(f) as estimateDataIn:
                            ...
                            pptEst = estimateDf['Participant Estimate']
                            advEst = estimateDf['Advisor Estimate']
                            avPptErrTrain = round(np.mean(pptErr.loc[(estimateDf['Trial Type'] == "training")]),3)
                            avPptErrAdj = round(np.mean(pptErr.loc[(estimateDf['Trial Type'] == "adjust")]),3)
                            avPptErrRew = round(np.mean(pptErr.loc[(estimateDf['Trial Type'] == "reward")]),3)
                            avAdvErrTrain = round(np.mean(advErr.loc[(estimateDf['Trial Type'] == "training")]),3)
                            avAdvErrAdj = round(np.mean(advErr.loc[(estimateDf['Trial Type'] == "adjust")]),3)
                            avAdvErrRew = round(np.mean(advErr.loc[(estimateDf['Trial Type'] == "reward")]),3)
                            avDistToAnch = round(abs(np.mean(pptEst.loc[(estimateDf['Trial Type'] == "adjust")]) - np.mean(advEst.loc[(estimateDf['Trial Type'] == "adjust")])),3)
                            estimateVars = [avPptErrTrain, avPptErrAdj, avPptErrRew, avAdvErrTrain, avAdvErrAdj, avAdvErrRew, avDistToAnch]
                            estimateHeads = ['Average Ppt Training Error', 'Average Ppt Adjust Error', 'Average Ppt Reward Error', 'Average Adv Training Error', 'Average Adv Adjust Error', 'Average Adv Reward Error', 'Average Distance to Anchor']

            if (miscFlag == 1):
                for subjectFile in os.listdir('../../private/Subjects'):
                    subjectName = date + '_' + pid + '_SUBJECT.csv'
                    if fnmatch.fnmatch(subjectFile, subjectName):
                        f = '../../private/Subjects/' + subjectFile
                        with open(f) as subjectDataIn:
                            subjectDf = pd.read_csv(subjectDataIn)
                            dotReward = subjectDf["dotReward"][0]
                            estReward = subjectDf["estimateReward"][0]
                            totReward = subjectDf["totalReward"][0]
                            condition = subjectDf["CBcondition"][0]
                            miscVars = [dotReward, estReward, totReward, condition]
                            miscHeads = ['dotReward', 'estimateReward', 'totalReward', 'condition']
            ...
            # One off variables (reward, experiment order,
            ...
            advisorChangedPercent = len(df.loc[df["advisorChanged"]==1])/changeTrials
            ...
            algorChoiceBlk4 = len(df.loc[(df["trialType"] == "choice") & (df["whichAdvisor"] == 1) & (df["block"] == 4)])/40
            ...
            blk3acc = round(blk3acc, 3)
            algorChoiceBlk4 = round(algorChoiceBlk4, 3)
            ...
            allVars = [pid, finalDD, numOfTrials, algorChoice, humanChoice, earlyHumanAcc, earlyAlgorAcc, laterHumanChoice, laterAlgorChoice, advisorChangedPercent, leftBoxChosen, preferenceStrength, maxCj, minCj, cjRange, numOfCjVals, adviceIgnoredTrials, meanCjDiff, resolution, resolution2, meanCor1, meanCor2, humanSway, algorSway, meanRt1, meanRt2, meanCtc, accQuant1, accQuant2, accQuant3, accQuant4, advQuant1, advQuant2, advQuant3, advQuant4, cjQuant1, cjQuant2, cjQuant3, cjQuant4, meanCj1, meanCj2, stDevCj1, stDevCj2, humanAgreedPercent, algorAgreedPercent, agreedDiff, humanAgreedConfDiff, algorAgreedConfDiff, humanDisagreedConfDiff, algorDisagreedConfDiff, algorRelativeInfluence, blk1dd, blk2dd, blk3dd, blk1acc, blk2acc, blk3acc, algorChoiceBlk4]
